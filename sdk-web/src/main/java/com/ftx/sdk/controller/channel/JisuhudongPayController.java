...
import com.ftx.sdk.model.JisuhudongPayModel;
...
@RestController
public class JisuhudongPayController {

    private Logger logger = LoggerFactory.getLogger(JisuhudongPayController.class);
    ...
    @RequestMapping(value = "/charge/jshd")
    public String callback(JisuhudongPayModel jisuhudongPayModel) {
        try {

            logger.debug("jisuhudong charge data:{}", gson.toJson(jisuhudongPayModel));
            logger.info("jisuhudong charge data:{}", gson.toJson(jisuhudongPayModel));
            //获取订单
            TSdkOrder charge = orderService.queueOrder(Long.parseLong(jisuhudongPayModel.getCp_order_no()));
            if (null == charge) {
                logger.error("jisuhudoong_pay_callback接口异常: [获取订单数据失败, requestBody={}, orderId={}]", gson.toJson(jisuhudongPayModel), jisuhudongPayModel.getCp_order_no());
                return FAILED;
            }

            //更新订单参数(channel_bill_num)
            charge.setChannelBillNum(jisuhudongPayModel.getOrder_no());

            //校验签名
            SdkParamCache paramCache = sdkService.getConfig(Integer.parseInt(charge.getPackageId()));
            String sign = jisuhudongPayModel.getSign();

            String channelAppKey = paramCache.channelConfig().get("channelAppKey");

            Map<String, String> map = MapsUtils.object2StringMap(jisuhudongPayModel);
            map.remove("sign");
            ...
            if (sign.equalsIgnoreCase(mySign)) {
                ...
                logger.info("jisuhudong_pay_callback 支付成功!");
                return SUCCESS;
            } else {
                orderService.orderUpdate(charge.getOrderId(), charge.getChannelBillNum(), ChargeStatus.PayFailed);
                logger.error("jisuhudong_pay_callback接口签名校验异常: sign:{}, mySign:{}, signStr:{}]", sign, mySign, signStr);
                return ErrorSign;
            }
        } catch (Exception e) {
            logger.error("jisuhudong_pay_callback接口异常: [服务器异常: requestBody={}, error_message={}]", gson.toJson(jisuhudongPayModel), e.getMessage(), e);
            return FAILED;
        }
